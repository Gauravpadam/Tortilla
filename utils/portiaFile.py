...
class SentenceSelection(BaseModel):
    claim: List[str] = Field(..., description="List of up to 10 fact-check-worthy sentences")
...
def build_article_plan():
    builder = (
        PlanBuilderV2("Extract a single fact-check-worthy claim from an article")
        ...
        # ----------------------------
        # Step 1: LLM extracts claim directly
        # ----------------------------
        .llm_step(
            step_name="Extract claim",
            task=(
                "You are an expert summarizer. "
                "Read the given article and extract exactly one verifiable claim "
                "⚠️ Output Rules:\n"
                "- Return only one claim.\n"
                "- Claim sentence should be around 10 words"
                "- Respond only in valid JSON format:\n"
                "{ \"claim\": \"[...]\" }\n"
                "- Do not include explanations, reasoning, or extra text."
            ),
            inputs=[Input("article_text")],
...
def article_analysis_service(article: str) -> List[FactCheckResults]:
    results = portia.run_plan(article_plan, plan_run_inputs={"article_text": article, "language": "en"})
    sentences = results.outputs.final_output.value.claim
...
def sentence_analysis_service(user_query:str) -> FactCheckResults:
    run = portia.run_plan(
        plan,
        plan_run_inputs={"user_query": user_query, "language": "en"}
    )
